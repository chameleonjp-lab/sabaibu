-- Allow verified Sabasaba scores to remain negative when deductions exceed additions.
-- All run, time, outcome, counter, and client-score consistency checks remain unchanged.
CREATE OR REPLACE FUNCTION public.submit_sabasaba_run(p_play_token uuid, p_mode text, p_outcome text, p_score integer, p_elapsed_seconds integer, p_kills integer, p_level integer, p_damage_hits integer, p_damage_taken integer, p_client_version text DEFAULT ''::text)
 RETURNS TABLE(accepted boolean, already_submitted boolean, result_score integer, result_best_score integer, result_play_count integer, is_new_best boolean, kill_points integer, time_points integer, level_points integer, hit_penalty integer, damage_penalty integer, positive_total integer, penalty_total integer)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
AS $function$
...
begin
  ...
  -- The client simulation clock stops during the countdown, pause screen,
  -- upgrade selection, boss rewards, and lifecycle/background suspension.
  -- Server time remains an upper bound: a client may not submit more active
  -- game time than the run has existed, but legitimate stopped time must not
  -- invalidate the recorded active game time.
  ...
  if p_elapsed_seconds > v_server_elapsed + 2 then
    raise exception 'elapsed time exceeds server time';
  end if;
  ...
  v_score := v_positive_total - v_penalty_total;
...
comment on function public.submit_sabasaba_run(
  uuid, text, text, integer, integer, integer, integer, integer, integer, text
) is 'Verified Sabasaba submission. Allows intentional gameplay pauses while rejecting client active time beyond server run time.';
