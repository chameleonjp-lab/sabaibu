 ...
 * The asymmetric HUD uses Siege Amber as the sole high-priority player-facing signal.
 */
...
export default function GameCanvas() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    createGameScene(engine, canvas, { demoMode, forceUpgrade, forceModulePreview, bossPreview, strikerPreview, rerollPreview, onSnapshot: setSnapshot }).then((handle) => {
      if (cancelled) {
        handle.dispose();
        return;
      }
  ...
  }, [demoMode, forceUpgrade, forceModulePreview, bossPreview, strikerPreview, rerollPreview]);
  ...
  return (
    <main className="game-shell" aria-label="Neon Siege Survivor">
      ...
      <section className="hud-layer" aria-live="polite">
        ...
        <footer className="loadout-rail">
          ...
          <div className="instruction"><kbd>W A S D</kbd><span>POSITION TO SURVIVE</span></div>
        </footer>
