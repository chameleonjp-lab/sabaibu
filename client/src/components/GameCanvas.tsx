...
export default function GameCanvas() {
  ...
  const openSettings = useCallback((fromPause = false) => {
    ...
    if (!fromPause) setPauseOpen(false);
  ...
  useEffect(() => {
    ...
    if (snapshot.phase === "paused") {
      setIsPaused(true);
      if (!settingsOpen && !tutorialOpen && !pausedBySettingsRef.current) setPauseOpen(true);
      return;
    }
  ...
  useEffect(() => {
    ...
    void createGameScene(engine, canvas, sceneOptions).then((handle) => {
      ...
      if (settingsOpenRef.current) handle.setPaused(true);
      setSceneReady(true);
